...
function void
RayTrace(scene* Scene, surface* Surface, s32 SamplesPerPixel, s32 MaxBounces, b32 DebugOn)
{
	...
	{
		random_sequence RNG = SeedRandom(4815162342ull*(Y + 1) + 1123581321ull);
...
main(int ArgCount, char** Args)
{
	...
	if (!Options.Error)
	{
		...
		if (Success)
		{
			...
			RayTrace(&Scene, &Surface, Options.SamplesPerPixel, Options.MaxBounces, Options.Debug);
